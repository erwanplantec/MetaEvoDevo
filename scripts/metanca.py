...
SAVE_FILE = "saves/metanca.pickle"
...
def main():
	...
	task = MetaEvolutionTask(
		statics, TARGETS, loss_fn, initializer, inner_es, 
		strategy_params=inner_es_params, inner_generations=INNER_GENS)
	
	outer_es = ex.DES(popsize=OUTER_POPSIZE, num_dims=params_shaper.total_params)
	outer_es_params = outer_es.default_params
	train_fn = EvosaxTrainer(
		task, outer_es, params_shaper, es_params=outer_es_params, gens=OUTER_GENS, n_repeats=OUTER_REPS)
